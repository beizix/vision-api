package io.vision.api.useCases.storage.uploadFile.adapters.web;
...
class UploadFileWebAdapterTest extends WebMvcTestBase {
  ...
  @MockitoBean
  private GetFileResourceUseCase getFileResourceUseCase;
  ...
  void upload_multipart_success() throws Exception {
    ...
    given(uploadFileUseCase.operate(eq(type), any(InputStream.class), anyString(), anyLong()))
        .willReturn(Optional.of(uploadFile));
    given(getFileResourceUseCase.operate(fileId)).willReturn(resourceUrl);
